...
export const AppLoader = React.memo(function AppLoader({
  ...
  return (
    ...
    >
      {/* Centered Main Stage */}
      <div className={`relative flex flex-col items-center justify-end w-72 h-44 overflow-hidden rounded-2xl ${scaleClass}`}>
        {/* Ambient Subtle Green Glow */}
        <div className="absolute top-4 h-32 w-52 rounded-full bg-emerald-100/40 blur-2xl pointer-events-none" />

        {/* ----------------- PARALLAX LAYER 1: MONOCHROME SKYLINE ----------------- */}
        {/* Sits right on top of the road surface */}
        <div className="absolute bottom-[17px] left-0 w-full h-28 overflow-hidden pointer-events-none z-0">
          <div className="flex w-[520px] animate-skyline-scroll opacity-85">
            {/* Seamless Repeating Skyline Segment A */}
            <svg viewBox="0 0 260 100" className="w-[260px] h-[100px] shrink-0" fill="none">
              <rect x="10" y="12" width="28" height="88" rx="2" fill="#f1f5f9" />
              <rect x="62" y="8" width="34" height="92" rx="2" fill="#f1f5f9" />
              <line x1="79" y1="2" x2="79" y2="8" stroke="#cbd5e1" strokeWidth="1.5" />
              <rect x="134" y="16" width="32" height="84" rx="2" fill="#f1f5f9" />
              <rect x="198" y="10" width="30" height="90" rx="2" fill="#f1f5f9" />

              <rect x="0" y="32" width="22" height="68" fill="#e2e8f0" />
              <rect x="30" y="24" width="36" height="76" rx="2" fill="#e2e8f0" />
              <circle cx="39" cy="34" r="1.5" fill="#ffffff" />
              <circle cx="48" cy="34" r="1.5" fill="#ffffff" />
              ...
              <rect x="88" y="20" width="48" height="80" rx="2" fill="#cbd5e1" />
              <rect x="85" y="18" width="54" height="3" fill="#94a3b8" />
              <rect x="96" y="26" width="12" height="8" rx="1" fill="#ffffff" opacity="0.9" />
              <rect x="116" y="26" width="12" height="8" rx="1" fill="#ffffff" opacity="0.9" />
              <rect x="96" y="40" width="12" height="8" rx="1" fill="#ffffff" opacity="0.7" />
              <rect x="116" y="40" width="12" height="8" rx="1" fill="#ffffff" opacity="0.7" />

              <rect x="156" y="28" width="22" height="72" rx="4" fill="#e2e8f0" />
              <ellipse cx="167" cy="28" rx="11" ry="3" fill="#cbd5e1" />

              <rect x="184" y="35" width="42" height="65" rx="2" fill="#cbd5e1" />
              <rect x="190" y="42" width="30" height="4" rx="0.5" fill="#94a3b8" />
              <rect x="236" y="40" width="24" height="60" fill="#e2e8f0" />
            </svg>

            {/* Seamless Repeating Skyline Segment B */}
            <svg viewBox="0 0 260 100" className="w-[260px] h-[100px] shrink-0" fill="none">
              <rect x="10" y="12" width="28" height="88" rx="2" fill="#f1f5f9" />
              <rect x="62" y="8" width="34" height="92" rx="2" fill="#f1f5f9" />
              <line x1="79" y1="2" x2="79" y2="8" stroke="#cbd5e1" strokeWidth="1.5" />
        ...
        {/* ----------------- SPEED PARTICLES ----------------- */}
        <div className="absolute top-10 left-2 z-10 animate-trail-leaf-1 pointer-events-none">
          <svg viewBox="0 0 20 20" className="h-3 w-3 text-emerald-500 drop-shadow-xs" fill="currentColor">
        ...
        <div className="absolute top-16 left-4 z-10 animate-trail-leaf-2 pointer-events-none">
          <svg viewBox="0 0 20 20" className="h-2.5 w-2.5 text-emerald-400 drop-shadow-xs" fill="currentColor">
            <path d="M10 2C5 2 2 7 2 12C2 17 7 18 10 18C15 18 18 13 18 8C18 3 15 2 10 2Z" />
          </svg>
        </div>
        <div className="absolute top-13 0 z-10 animate-trail-citrus pointer-events-none">
          <svg viewBox="0 0 24 24" className="h-3 w-3 drop-shadow-xs" fill="none">
        ...
        {/* ----------------- LAYER 2: CARGO TRUCK PLANTED ON ROAD ----------------- */}
        {/* Zero air-gap: viewBox trimmed precisely to bottom of wheels (y=82) with negative overlap */}
        <div className="relative z-20 -mb-[1px] w-52 pointer-events-none">
          <svg
          ...
          >
            ...
            {/* A. SUSPENSION-BOUNCING TRUCK BODY (Wheels stay planted below) */}
            <g className="animate-body-suspension">
              {/* Headlight Beam */}
              ...
              {/* CAFKART SPLASH LOGO: CENTERED FORWARD, ZERO CLIPPING[span_0](start_span)[span_0](end_span) */}
            ...
            {/* B. GROUNDED ALLOY WHEELS (Firmly seated at y=68 with r=14 -> bottom=82) */}
        ...
        {/* ----------------- LAYER 3: SOLID EDGE-TO-EDGE HIGH-CONTRAST ROADWAY ----------------- */}
        <div className="relative w-full z-30 flex flex-col items-center">
          {/* Concrete Road Curb Line (Tires sit flush on this) */}
          <div className="h-[2px] w-full bg-[#64748b]" />
          ...
          <div className="h-3 w-full bg-[#1e293b] flex items-center justify-center overflow-hidden shadow-inner">
            ...
            <div className="animate-road-speed h-[2px] w-full bg-[repeating-linear-gradient(90deg,#ffffff_0px,#ffffff_14px,transparent_14px,transparent_28px)] opacity-90" />
          </div>
          {/* Road Bottom Border */}
        ...
        <div className="h-1 w-48 rounded-full bg-slate-500/25 blur-2xs animate-shadow-run mt-0.5" />
      </div>
      ...
      <style>{`
        ...
        /* Suspends only the upper truck body on its springs; tyres stay planted */
        @keyframes bodySuspension {
          0%, 100% { transform: translateY(0); }
          25% { transform: translateY(-1.2px) rotate(-0.25deg); }
          50% { transform: translateY(0.4px) rotate(0.15deg); }
          75% { transform: translateY(-0.8px) rotate(-0.1deg); }
        }
        .animate-body-suspension {
          animation: bodySuspension 0.65s ease-in-out infinite;
        }
        ...
        @keyframes shadowRun {
          ...
          50% { transform: scaleX(0.94); opacity: 0.18; }
        }
        .animate-shadow-run {
