...
export function CartItem({ item, onIncrement, onDecrement, onRemove, onClick }: CartItemProps) {
  return (
    <div className="flex gap-3 p-3 bg-white border border-ink-100 rounded-2xl shadow-card">
      <button onClick={onClick} className="h-[76px] w-[76px] rounded-xl overflow-hidden bg-ink-50 shrink-0">
        <CachedImage src={item.product.image} alt={item.product.name} className="h-full w-full object-cover" />
      </button>
      ...
      <button onClick={onRemove} className="self-start p-1 text-ink-300 hover:text-red-500 transition-colors" aria-label="Remove item">
        <Trash2 size={15} />
      </button>
    </div>
  );
