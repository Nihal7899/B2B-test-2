...
export function SplashScreen({ onFinish, isReady = false }: SplashScreenProps) {
  ...
  return (
    ...
    >
      {/* Restored glow using the new green */}
      <div className="absolute -top-32 left-1/2 h-96 w-96 -translate-x-1/2 rounded-full bg-[#89c74e]/10 blur-[130px]" />
      ...
      <div className="relative w-full h-full select-none">
        ...
        >
          {/* Replaced old mint with new green for the loading bar gradient */}
          <div className="splash-bar h-full rounded-full bg-gradient-to-r from-[#89c74e] via-[#89c74e]/60 to-white" />
        </div>

        <p
          className={`absolute bottom-6 left-1/2 -translate-x-1/2 text-[10px] font-semibold tracking-wider text-[#89c74e]/80 uppercase w-full text-center transition-opacity duration-700 delay-300 transform-gpu ${
            showText ? 'opacity-100' : 'opacity-0'
          }`}
        >
