...
export const StoreCard = React.memo(
  function StoreCard({
    ...
    return (
      <button
        ...
        className="group relative h-[290px] w-[225px] shrink-0 overflow-hidden rounded-[24px] bg-white text-left shadow-[0_6px_22px_rgba(15,23,42,0.1)] active:scale-[0.985] transform-gpu"
      >
        {/* Image Section */}
        <div className="absolute inset-x-0 top-0 h-[162px] overflow-hidden bg-slate-100">
          ...
          {/* Replaced mix-blend-multiply with hardware-accelerated opacity */}
          <div
        ...
        <div className="absolute inset-x-0 bottom-0 h-[153px] bg-white">
          ...
          <div className="relative z-10 h-full px-3.5 pb-3.5 pt-1.5">
            ...
            {/* Shop Now Button */}
            <div
              className="mt-1.5 flex h-[29px] items-center justify-between rounded-xl px-3 text-white shadow-sm"
  ...
  (prev, next) => prev.store.id === next.store.id
);

export const StoreCarousel = React.memo(
  ...
  (prev, next) => {
    ...
    return prev.stores.every((s, i) => s.id === next.stores[i]?.id);
  }
);