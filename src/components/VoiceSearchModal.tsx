...
// Sculpted static wave heights to match the symmetrical design
    ...
    >
      ...
      >
        ...
        <button
          ...
          className="absolute top-4 right-4 h-9 w-9 rounded-full bg-slate-100/80 hover:bg-slate-200 backdrop-blur-sm flex items-center justify-center text-slate-500 active:scale-90 transition-all z-20"
        >
        ...
        <div className="relative z-10 flex flex-col items-center justify-center h-full w-full px-6 pt-4 pb-12">
          ...
          <div 
            ...
            className="flex items-center justify-center gap-4 h-[100px] w-full vs-wave-container transition-transform duration-200"
          >
            ...
            {/* Mic Center - Clickable to start/retry listening */}
            <div 
              className="relative flex items-center justify-center shrink-0 w-28 h-28 rounded-full cursor-pointer hover:scale-[1.02] active:scale-95 transition-transform z-20"
              onClick={onRetry} 
              role="button" 
            ...
            >
              ...
              <div className="absolute w-[90px] h-[90px] rounded-full bg-[#0d5235]/5 flex items-center justify-center pointer-events-none"></div>
              
              {/* Inner dark forest green mic button */}
              <div
                className={`relative w-[68px] h-[68px] rounded-full flex items-center justify-center shadow-lg transition-all duration-300 ${
                  showError
                    ? 'bg-red-600'
              ...
              >
                {showError ? (
                  <AlertCircle size={28} className="text-white" strokeWidth={2.2} />
                ) : isListening ? (
                  <MicOff size={28} className="text-white" strokeWidth={2.2} />
                ) : (
                  <Mic size={28} className="text-white" strokeWidth={2.2} />
                )}
              </div>
            </div>
          ...
          {/* Texts */}
          <h2 className="mt-4 text-[24px] font-bold text-[#112c22] tracking-tight text-center">
            {title}
          </h2>
          <p className="mt-2 text-[14px] font-medium text-[#4f6b5f] text-center leading-relaxed whitespace-pre-line">
            {subtitle}
          </p>
        ...
        <style>{`
          /* Expanding mic rings */
          .vs-ring {
            border: 1.5px solid rgba(13, 82, 53, 0.4);
