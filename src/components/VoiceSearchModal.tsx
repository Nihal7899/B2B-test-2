            ...
            {webGpuSupported ? (
              ...
              <>
                <div className={`flex items-center gap-1.5 transition-opacity ${isListening && !showError ? 'opacity-100' : 'opacity-0'}`}>
                  {WAVE_BARS_LEFT.map((b, i) => (
                    <div 
                      key={`l-${i}`} 
                      className="w-1 rounded-full bg-[#0d5235] animate-pulse" 
                ...
                <div 
                  onClick={onRetry} 
                  className={`relative flex items-center justify-center w-[68px] h-[68px] rounded-full shadow-lg cursor-pointer transition-all ${showError ? 'bg-red-600' : 'bg-[#0d5235] animate-pulse'}`}
                >
