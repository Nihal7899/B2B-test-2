...
function SectionHeader({
  icon: Icon, title, subtitle, step, action,
}: {
  icon: any; title: string; subtitle?: string; step?: number; action?: React.ReactNode;
}) {
  return (
    <div className="flex items-center gap-3 px-5 pt-5 pb-4">
      <div className="relative shrink-0">
        <div className="h-10 w-10 rounded-2xl bg-gradient-to-br from-[#02402c] to-[#046b4a] flex items-center justify-center shadow-md shadow-[#02402c]/25 ring-1 ring-white/20">
          <Icon size={17} className="text-white" strokeWidth={2.4} />
        </div>
        {step !== undefined && (
          <span className="absolute -top-1 -right-1 h-[18px] w-[18px] rounded-full bg-white border-[2.5px] border-[#02402c] text-[#02402c] text-[9px] font-black flex items-center justify-center shadow-sm">
            {step}
          </span>
        )}
      </div>
      <div className="flex-1 min-w-0">
        <h2 className="text-[15.5px] font-black text-slate-900 tracking-[-0.01em]">{title}</h2>
        {subtitle && (
          <p className="text-[11.5px] text-slate-400 font-semibold mt-0.5 truncate">{subtitle}</p>
        )}
      </div>
      {action}
    </div>
...
export function CheckoutScreen({ cart, onBack, onOrderPlaced, onAddAddress }: CheckoutScreenProps) {
  ...
  const handleApplyPromo = async () => {
    ...
    setPromoError(null);
    const result = await cart.applyPromo(promoInput.trim().toUpperCase());
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col">
        <div className="bg-gradient-to-br from-[#02402c] via-[#03573b] to-[#02402c] safe-top px-4 pt-4 pb-6">
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="flex flex-col items-center gap-3">
            ...
            <p className="text-[11.5px] font-bold text-slate-400 tracking-wide">Preparing checkout...</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-[#f6f7f9] pb-6 scroll-smooth [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
      {/* ==================== STICKY HEADER ==================== */}
      <header className={`sticky top-0 z-50 transition-all duration-300 ${scrolled ? 'shadow-[0_10px_30px_-12px_rgba(2,64,44,0.35)]' : ''}`}>
        <div className="relative bg-gradient-to-br from-[#02402c] via-[#03573b] to-[#013a29] safe-top overflow-hidden">
          {/* decorative glows */}
          <div className="pointer-events-none absolute -top-24 -right-20 h-56 w-56 rounded-full bg-emerald-400/20 blur-3xl" />
          <div className="pointer-events-none absolute -bottom-28 -left-16 h-56 w-56 rounded-full bg-emerald-500/15 blur-3xl" />
          {/* subtle dot grid */}
          <div
          ...
          <div className={`relative px-4 pt-4 transition-all duration-300 ${scrolled ? 'pb-4' : 'pb-7'}`}>
            <div className="flex items-center gap-3">
              <button
                onClick={onBack}
                className="h-10 w-10 rounded-2xl bg-white/10 border border-white/15 flex items-center justify-center backdrop-blur-md active:scale-95 active:bg-white/20 transition-all hover:bg-white/15"
                aria-label="Go back"
              >
              ...
              <div className="flex-1 min-w-0">
                <h1 className="text-[20px] font-black tracking-[-0.02em] text-white leading-tight">Checkout</h1>
                <p className="text-[11px] text-emerald-100/70 font-semibold mt-0.5 truncate tracking-wide">
                  {scrolled
              ...
              <div className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-full bg-white/10 border border-white/15 backdrop-blur-md">
                <ShieldCheck size={12} className="text-emerald-300" strokeWidth={2.6} />
                <span className="text-[10px] font-black text-white tracking-wide">SECURE</span>
              </div>
            </div>

            {/* compact total chip appears when scrolled */}
            <div className={`overflow-hidden transition-all duration-300 ease-out ${scrolled ? 'max-h-14 opacity-100 mt-3' : 'max-h-0 opacity-0 mt-0'}`}>
              <div className="flex items-center justify-between rounded-2xl bg-white/10 border border-white/15 backdrop-blur-md px-3.5 py-2.5">
                <span className="text-[11px] font-bold text-emerald-100/90 tracking-wide">Grand Total</span>
      ...
      <div className="px-4 -mt-3 relative z-10">
        <div className="bg-white rounded-[26px] p-1.5 shadow-[0_20px_50px_-20px_rgba(2,64,44,0.4)] border border-white/60 flex gap-1.5 backdrop-blur-sm">
          <button
            onClick={() => handleDeliveryTypeChange('standard')}
            className={`flex-1 h-[78px] rounded-[20px] flex flex-col items-center justify-center gap-[1px] transition-all duration-300 relative overflow-hidden ${
              deliveryType === 'standard'
                ? 'bg-gradient-to-b from-[#02402c]/[0.09] to-[#02402c]/[0.02] ring-1 ring-[#02402c]/15'
                : 'hover:bg-slate-50/80'
            }`}
          >
            {deliveryType === 'standard' && (
              <span className="absolute top-2 right-2 h-[18px] w-[18px] rounded-full bg-[#02402c] flex items-center justify-center shadow-sm">
                <Check size={10} className="text-white" strokeWidth={4} />
              </span>
            )}
            <StandardModeIcon active={deliveryType === 'standard'} />
            <span className={`font-black text-[14px] mt-1 tracking-[-0.01em] ${deliveryType === 'standard' ? 'text-[#02402c]' : 'text-slate-600'}`}>
              Standard
            </span>
            <span className={`text-[10.5px] font-bold ${deliveryType === 'standard' ? 'text-[#02402c]/70' : 'text-slate-400'}`}>
              Next day delivery
            </span>
          </button>
          ...
          >
            ...
            <span className={`font-black text-[14px] mt-1 tracking-[-0.01em] ${deliveryType === 'express' ? 'text-[#02402c]' : 'text-slate-600'}`}>
              Express
            </span>
            <span className={`text-[10.5px] font-bold truncate px-1 max-w-full ${deliveryType === 'express' ? 'text-[#02402c]/70' : 'text-slate-400'}`}>
      ...
      <div className="px-4 pt-6 space-y-4">
        {/* ==================== BILLING DETAILS ==================== */}
        <section className="bg-white rounded-[26px] border border-slate-100 shadow-[0_10px_36px_-18px_rgba(15,23,42,0.15)] overflow-hidden">
          <SectionHeader
            icon={Building2}
            title="Billing Details"
            subtitle="GST invoice information"
            step={1}
          />

          <div className="px-4 pb-4">
            {businesses.length === 0 ? (
              <div className="rounded-2xl bg-gradient-to-br from-amber-50 to-amber-50/40 border border-amber-200/70 p-4 flex gap-3">
                    ...
                    <button
                      ...
                      className={`w-full text-left p-4 rounded-2xl border-2 transition-all duration-200 relative overflow-hidden ${
                        isSel
                          ...
                          : 'border-slate-150 border-slate-200 bg-white hover:border-slate-300'
                      }`}
                    >
                      <div className="flex items-start gap-3">
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-2 flex-wrap">
                            <p className="text-[15px] font-black text-slate-900 tracking-[-0.01em]">{b.business_name}</p>
                            {b.is_default && (
                          ...
                          {b.gst_registered && b.gstin ? (
                            ...
                            <p className="text-[11.5px] text-slate-400 mt-1.5 font-medium">Not GST registered</p>
                          )}
                        </div>
                      </div>
                    </button>
        ...
        <section className="bg-white rounded-[26px] border border-slate-100 shadow-[0_10px_36px_-18px_rgba(15,23,42,0.15)] overflow-hidden">
          <SectionHeader
            ...
            subtitle={addresses.length > 0 ? `${addresses.length} saved address${addresses.length > 1 ? 'es' : ''}` : undefined}
            step={2}
            action={
              addresses.length > 0 ? (
                <button
                  onClick={onAddAddress}
                  className="flex items-center gap-1 text-[11.5px] font-black text-[#02402c] bg-[#02402c]/[0.07] hover:bg-[#02402c]/[0.12] px-2.5 py-1.5 rounded-lg transition-colors active:scale-95"
                >
          ...
          <div className="px-4 pb-4">
            {addresses.length === 0 ? (
              <button
                onClick={onAddAddress}
                className="w-full h-24 rounded-2xl border-2 border-dashed border-[#02402c]/25 bg-gradient-to-br from-[#02402c]/[0.04] to-transparent text-[#02402c] text-sm font-bold flex items-center justify-center gap-2.5 hover:bg-[#02402c]/[0.06] transition-colors active:scale-[0.99]"
              >
                <div className="h-9 w-9 rounded-xl bg-[#02402c]/10 flex items-center justify-center">
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-2">
                            <p className="text-[15px] font-black text-slate-900 tracking-[-0.01em]">{addr.label}</p>
                            {addr.is_default && (
                              <span className="text-[9.5px] font-black bg-[#02402c]/10 text-[#02402c] rounded-md px-2 py-0.5 tracking-wide">
                                DEFAULT
                              </span>
                          ...
                          <p className="text-[12.5px] text-slate-600 mt-1.5 leading-relaxed font-medium">
                          ...
                          <div className="flex items-center gap-2 mt-2.5 flex-wrap">
                            ...
                            <span className="text-[11px] font-semibold text-slate-500">{addr.phone}</span>
                          </div>
                        </div>
                      </div>
                    </button>
        ...
        {/* ==================== OFFERS & PROMO ==================== */}
        <section className="bg-white rounded-[26px] border border-slate-100 shadow-[0_10px_36px_-18px_rgba(15,23,42,0.15)] overflow-hidden">
          <SectionHeader icon={Tag} title="Offers & Benefits" subtitle="Apply coupons to save more" step={3} />
          <div className="px-4 pb-4">
            {cart.appliedPromo ? (
              <div className="relative flex items-center gap-3 rounded-2xl border-2 border-dashed border-emerald-300 bg-gradient-to-br from-emerald-50 to-emerald-50/30 p-4 overflow-hidden">
                <div className="pointer-events-none absolute -top-8 -right-8 h-24 w-24 rounded-full bg-emerald-200/40 blur-2xl" />
                <div className="relative h-10 w-10 rounded-xl bg-emerald-100 flex items-center justify-center shrink-0">
                  <Percent size={18} className="text-emerald-600" strokeWidth={2.6} />
                </div>
                <div className="flex-1 min-w-0 relative">
                  ...
                  <p className="text-[11.5px] font-bold text-emerald-600 mt-0.5">
                    You saved ₹{cart.appliedPromo.discount.toLocaleString('en-IN')} 🎉
                  </p>
                </div>
                <button
                  onClick={() => { cart.clearPromo(); }}
                  className="relative h-9 w-9 rounded-xl bg-white border border-emerald-200 text-emerald-600 flex items-center justify-center active:scale-95 transition-transform shadow-sm"
                ...
                <div className="flex gap-2.5">
                  <div className="relative flex-1">
                    ...
                    <input
                      ...
                      autoCorrect="off"
                      spellCheck={false}
                      className="w-full h-12 rounded-xl border border-slate-200 pl-10 pr-3 text-[13px] font-bold uppercase tracking-wider outline-none focus:border-[#02402c] focus:ring-2 focus:ring-[#02402c]/10 transition-all bg-slate-50/50 focus:bg-white placeholder:text-slate-300 placeholder:tracking-wider placeholder:font-bold"
                    />
                  </div>
                  <button
                    onClick={handleApplyPromo}
                    disabled={applyingPromo || !promoInput.trim()}
                    className="h-12 px-5 rounded-xl bg-gradient-to-b from-[#02402c] to-[#03573b] text-white text-[13px] font-black flex items-center gap-2 disabled:from-slate-300 disabled:to-slate-300 disabled:cursor-not-allowed transition-all active:scale-95 shadow-md shadow-[#02402c]/20 disabled:shadow-none"
                  >
                    {applyingPromo ? <Loader2 size={16} className="animate-spin" /> : 'Apply'}
                  </button>
                </div>
                {promoError && (
                  <div className="flex items-center gap-2 mt-2.5 ml-1 animate-in fade-in slide-in-from-top-1 duration-200">
                    <AlertCircle size={13} className="text-red-500" />
                    <p className="text-[11.5px] text-red-500 font-bold">{promoError}</p>
                  </div>
                )}
              </>
            )}
        ...
        {/* ==================== WALLET ==================== */}
        <section className="bg-gradient-to-br from-[#02402c] via-[#03573b] to-[#013a29] rounded-[26px] shadow-[0_16px_40px_-22px_rgba(2,64,44,0.7)] overflow-hidden relative">
          <div className="pointer-events-none absolute -top-12 -right-12 h-44 w-44 rounded-full bg-emerald-400/20 blur-3xl" />
          <div className="pointer-events-none absolute -bottom-16 -left-10 h-44 w-44 rounded-full bg-emerald-500/15 blur-3xl" />
          <div className="pointer-events-none absolute top-0 inset-x-0 h-px bg-gradient-to-r from-transparent via-white/25 to-transparent" />

          <div className="relative p-4 flex items-center gap-3">
            <div className="h-11 w-11 rounded-2xl bg-white/10 border border-white/15 backdrop-blur-md flex items-center justify-center shrink-0 shadow-inner">
              <Wallet size={20} className="text-emerald-200" strokeWidth={2.4} />
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-[14px] font-black text-white tracking-[-0.01em] flex items-center gap-1.5">
                CafKart Wallet
                <Sparkles size={12} className="text-emerald-300" />
              </p>
              <p className="text-[12px] text-emerald-100/85 mt-0.5 font-semibold">
          ...
          {walletDeduction > 0 && (
            <div className="relative mx-4 mb-4 flex items-center justify-between rounded-xl bg-white/10 border border-white/15 backdrop-blur-md px-3.5 py-2.5 animate-in fade-in slide-in-from-top-1 duration-300">
              <span className="text-[11.5px] font-bold text-emerald-100/90 tracking-wide">Applying from wallet</span>
              <span className="text-[14px] font-black text-emerald-200 tabular-nums">
        ...
        {!isFullWalletPayment && (
          <section className="bg-white rounded-[26px] border border-slate-100 shadow-[0_10px_36px_-18px_rgba(15,23,42,0.15)] overflow-hidden">
            <SectionHeader
              ...
              subtitle={walletDeduction > 0 ? 'Choose how to pay the balance' : 'Pick your preferred option'}
              step={4}
              action={
                walletDeduction > 0 ? (
                  <span className="text-[14px] font-black text-[#02402c] tabular-nums">
            ...
            <div className="px-4 pb-4 grid grid-cols-2 gap-3">
              ...
              >
                {paymentMethod === 'cod' && (
                  <span className="absolute top-2.5 right-2.5 h-5 w-5 rounded-full bg-[#02402c] flex items-center justify-center shadow-sm">
                ...
                <p className="text-[13.5px] font-black text-slate-900 mt-3 tracking-[-0.01em]">Cash on delivery</p>
                <p className="text-[11px] text-slate-500 mt-0.5 font-semibold">Pay on arrival</p>
              </button>
              ...
              >
                ...
                <p className="text-[13.5px] font-black text-slate-900 mt-3 tracking-[-0.01em]">Online payment</p>
                <p className="text-[11px] text-slate-500 mt-0.5 font-semibold">UPI, Cards, NetBanking</p>
              </button>
            </div>
          </section>
        )}
        ...
        <section className="bg-white rounded-[26px] border border-slate-100 shadow-[0_10px_36px_-18px_rgba(15,23,42,0.15)] overflow-hidden">
          <SectionHeader
            icon={Package}
            title="Order Summary"
            subtitle={`${cart.items.length} item${cart.items.length > 1 ? 's' : ''} in your order`}
          />
          ...
          <div className="px-5 pb-5 space-y-4">
              ...
              {cart.items.map((item) => (
                <div key={item.product.id} className="flex items-center justify-between gap-3">
                  <div className="flex items-center gap-3 min-w-0 flex-1">
                    <div className="h-9 w-9 rounded-xl bg-gradient-to-br from-[#02402c]/10 to-[#02402c]/[0.03] border border-[#02402c]/10 flex items-center justify-center shrink-0">
                      <span className="text-[11.5px] font-black text-[#02402c]">{item.quantity}×</span>
                    </div>
                    <div className="min-w-0">
                      <p className="text-[12.5px] font-bold text-slate-800 truncate tracking-[-0.005em]">
                  ...
                  <span className="text-[13px] font-black text-slate-900 shrink-0 tabular-nums">
            ...
            <div className="flex justify-between items-end">
              ...
              <span className="text-[20px] font-black text-[#02402c] tracking-[-0.02em] tabular-nums">
                ₹{grandTotal.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
              </span>
            </div>
          </div>
        ...
        <div className="flex items-center justify-center gap-4 py-2">
          <div className="flex items-center gap-1.5">
            <ShieldCheck size={13} className="text-[#02402c]" strokeWidth={2.6} />
            <span className="text-[10.5px] font-bold text-slate-500 tracking-wide">Secure Payments</span>
          </div>
          <div className="h-3 w-px bg-slate-300" />
          <div className="flex items-center gap-1.5">
            <Truck size={13} className="text-[#02402c]" strokeWidth={2.6} />
            <span className="text-[10.5px] font-bold text-slate-500 tracking-wide">Fast Delivery</span>
          </div>
          <div className="h-3 w-px bg-slate-300" />
          <div className="flex items-center gap-1.5">
            <BadgeCheck size={13} className="text-[#02402c]" strokeWidth={2.6} />
            <span className="text-[10.5px] font-bold text-slate-500 tracking-wide">Verified Sellers</span>
          </div>
        </div>
      </div>
      ...
      <div className="sticky bottom-0 z-40 mt-4">
        <div className="px-4 pb-4 pt-3 bg-gradient-to-t from-[#f6f7f9] via-[#f6f7f9]/95 to-transparent backdrop-blur-[2px]">
          ...
          >
            ...
            {placing ? (
              ...
              <>
                ...
                <span className="flex items-center gap-2 bg-white/15 border border-white/15 px-3 py-1.5 rounded-xl backdrop-blur-sm relative">
          ...
          {error && (
            <div className="flex items-center justify-center gap-2 mt-2.5 animate-in fade-in slide-in-from-top-1 duration-200">
              <AlertCircle size={13} className="text-red-500" />
              <p className="text-[11.5px] text-red-500 font-bold">{error}</p>
            </div>
          )}
        </div>
      </div>
      ...
      {showPaymentAlert && (
        ...
        >
          <div
            className="bg-white rounded-[28px] p-6 max-w-sm w-full shadow-2xl animate-in zoom-in-95 duration-200 relative overflow-hidden"
