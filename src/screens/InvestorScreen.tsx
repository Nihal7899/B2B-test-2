...
// ─── FULLY RESTORED BRAND MARK WITH UNIQUE IDs ───────────────────────

function CafKartLogo({ className = 'h-9 w-9', gradId = 'ckGrad' }: { className?: string, gradId?: string }) {
  return (
...
export function InvestorScreen({ onBack }: { onBack?: () => void }) {
  ...
  const fetchMetrics = async () => {
    ...
    try {
      ...
      const activeCustomers30d = Number(data.activeCustomers30d) || 0;
      ...
      setMetrics({
        ...
        activeCustomers: activeCustomers30d,
        ...
        totalDiscounts,
        totalDeliveryFees,
        arpu: activeCustomers30d > 0 ? monthlySales / activeCustomers30d : 0,
        discountRate: monthlySales > 0 ? (totalDiscounts / (monthlySales + totalDiscounts)) * 100 : 0,
    ...
    <div className="relative min-h-screen bg-[#060A11] text-slate-100 selection:bg-emerald-500/30 [-webkit-tap-highlight-color:transparent]">
      ...
      <aside className="fixed inset-y-0 left-0 z-40 hidden w-[248px] flex-col border-r border-white/[0.06] bg-[#070C14]/80 backdrop-blur-2xl lg:flex">
        <div className="flex items-center gap-2.5 px-5 py-6">
          <CafKartLogo className="h-9 w-9 drop-shadow-[0_0_12px_rgba(89,217,182,0.35)]" gradId="desktop-ck-logo" />
        ...
        <div className="mt-auto space-y-3 p-3">
          <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-emerald-500/10 via-emerald-500/[0.03] to-transparent p-4">
        ...
        <div className="flex items-center justify-between gap-3 px-4 pb-3 pt-[max(0.75rem,env(safe-area-inset-top))]">
          <div className="flex min-w-0 items-center gap-2">
            {onBack && (
              <button
                onClick={onBack}
                aria-label="Go back"
                className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-white/[0.04] text-slate-300 transition active:scale-95"
              >
                <ArrowLeft size={15} />
              </button>
            )}
            ...
            <span className="ml-0.5 shrink-0 rounded-full bg-emerald-500/10 px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider text-emerald-300">
              Investor
            </span>
          </div>
          ...
          <div className="flex shrink-0 items-center gap-2">
            <button
              ...
              aria-label="Refresh"
              className="flex h-8 w-8 items-center justify-center rounded-lg bg-white/[0.04] text-emerald-300 transition active:scale-95"
            >
            ...
            <button
              ...
              aria-label="Exit"
              className="flex h-8 w-8 items-center justify-center rounded-lg bg-rose-500/10 text-rose-300 transition active:scale-95"
            >
              <LogOut size={14} />
            </button>
          </div>
        ...
        <main className="relative mx-auto max-w-[1400px] px-4 pb-28 pt-5 sm:px-6 lg:px-8 lg:pb-14 lg:pt-8">
          ...
          <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
            ...
            <div className="hidden items-center gap-2 lg:flex">
              <div className="flex items-center gap-2 rounded-xl bg-white/[0.03] px-3.5 py-2">
              ...
              <button
                onClick={fetchMetrics}
                className="group flex items-center gap-2 rounded-xl bg-white/[0.03] px-4 py-2 text-[11px] font-bold text-slate-300 transition-all hover:bg-emerald-500/10 hover:text-emerald-300 active:scale-[0.97]"
              >
  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
         <MetricCard
          label="ARPU (30d)"
          value={<AnimatedNumber value={metrics.arpu} format={formatCurrency} />}
          icon={Users}
          accent={ACCENT.emerald}
        ...
        />
         <MetricCard
          label="Discount Rate"
          value={<AnimatedNumber value={metrics.discountRate} format={(n) => `${n.toFixed(1)}%`} />}
          icon={Percent}
          accent={ACCENT.amber}
          hint="% of gross revenue lost"
        />
         <MetricCard
          label="Delivery Ratio"
          value={<AnimatedNumber value={metrics.monthlySales > 0 ? (metrics.totalDeliveryFees / metrics.monthlySales)*100 : 0} format={(n) => `${n.toFixed(1)}%`} />}
          icon={Package}
          accent={ACCENT.blue}
          hint="% of net revenue"
        />
...
function MoreTab({ onBack }: { onBack?: () => void }) {
  ...
  return (
    <div className="space-y-5">
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-emerald-500/[0.09] via-white/[0.02] to-transparent p-8 text-center sm:p-12">
        ...
        <div className="relative mx-auto flex h-16 w-16 items-center justify-center rounded-2xl bg-emerald-500/10 text-emerald-400 shadow-[0_0_40px_-8px_rgba(16,185,129,0.6)]">
          <Sparkles size={28} />
        </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        {features.map((f) => {
          ...
          return (
            <div
              key={f.title}
              className="group relative overflow-hidden rounded-2xl bg-gradient-to-b from-white/[0.04] to-white/[0.01] p-5 transition-all duration-300 hover:bg-white/[0.06]"
            >
              <div
      ...
      {onBack && (
        <button
          onClick={onBack}
          className="mx-auto flex items-center gap-2 rounded-xl bg-white/[0.03] px-5 py-2.5 text-xs font-bold text-slate-300 transition-all hover:bg-white/[0.06] hover:text-white active:scale-95"
        >
...
function MetricCard({
  ...
  return (
    <div className="group relative overflow-hidden rounded-2xl bg-gradient-to-b from-white/[0.045] to-white/[0.012] p-5 transition-all duration-300 hover:bg-white/[0.06] hover:shadow-[0_24px_60px_-30px_rgba(0,0,0,0.9)]">
      <div
...
function Panel({
  ...
  return (
    <section
      className={`relative overflow-hidden rounded-2xl bg-gradient-to-b from-white/[0.04] to-white/[0.012] backdrop-blur-xl ${className}`}
    >
...
function HighlightRow({
  ...
  return (
    <div className="flex items-center gap-3 rounded-xl bg-white/[0.02] px-3 py-2.5 transition-colors hover:bg-white/[0.04]">
      <span
        className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg"
        style={{ background: `${accent}1f`, color: accent }}
      >
...
function ChartTooltip({ active, payload, label }: any) {
  ...
  return (
    <div className="rounded-xl bg-[#0B111A]/95 px-3.5 py-2.5 shadow-2xl backdrop-blur-xl">
      <p className="text-[9px] font-bold uppercase tracking-wider text-slate-500">{label}</p>
    ...
    <div className="space-y-5">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {Array.from({ length: 4 }).map((_, i) => (
          <div key={i} className="h-[132px] animate-pulse rounded-2xl bg-white/[0.03]" />
        ))}
      </div>
      <div className="grid grid-cols-1 gap-5 xl:grid-cols-3">
        <div className="h-[380px] animate-pulse rounded-2xl bg-white/[0.03] xl:col-span-2" />
        <div className="h-[380px] animate-pulse rounded-2xl bg-white/[0.03]" />
      </div>
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-2">
...
function EmptyState({ onRetry }: { onRetry: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center gap-4 rounded-2xl bg-white/[0.02] py-20">
      ...
      <button
        onClick={onRetry}
        className="mt-1 flex items-center gap-2 rounded-xl bg-emerald-500/10 px-4 py-2 text-xs font-bold text-emerald-300 transition-all hover:bg-emerald-500/20 active:scale-95"
      >
