...
import { ArrowLeft, MapPin, Package, Truck, CheckCircle2, Clock, XCircle, Printer, Loader2 } from 'lucide-react';
  ...
  if (!order) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[50vh]">
        ...
        <button onClick={onBack} className="mt-3 text-sm font-bold text-brand-600">Go back</button>
      </div>
    );
  }
              ...
              return (
                <div key={step.key} className="flex items-center gap-3">
                  ...
                  {index < statusSteps.length - 1 && (
                    <div className={`absolute left-[27px] w-0.5 h-3 ${isDone ? 'bg-brand-600' : 'bg-ink-200'}`} />
                  )}
                </div>
              );
            })}
